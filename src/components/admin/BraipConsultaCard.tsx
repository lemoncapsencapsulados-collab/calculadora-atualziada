...
interface Resultado {
  ...
  itens: Array<{ codigo: string; produto: string; cliente: string; data_finalizacao: string | null; valor: number; comissao: number }>;
}

export function BraipConsultaCard() {
  ...
  const etapas = [
    { label: 'Autenticando na Braip', icon: KeyRound },
    { label: 'Buscando vendas do período', icon: Database },
    { label: 'Filtrando aprovadas e por produto', icon: Search },
  ...
  const { data: salvas = [] } = useQuery({
    queryKey: ['braip-consultas-salvas', consultorId],
    queryFn: async () => {
      let q = supabase.from('braip_consultas_salvas' as any).select('*').order('created_at', { ascending: false }).limit(50);
  ...
  const excluir = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await supabase.from('braip_consultas_salvas' as any).delete().eq('id', id);
      if (error) throw error;
    },
    onSuccess: () => {
      toast({ title: 'Registro excluído' });
      queryClient.invalidateQueries({ queryKey: ['braip-consultas-salvas'] });
    },
  });
  ...
  const consultar = async () => {
    ...
    iniciarProgresso();
    try {
      const { data: resp, error } = await supabase.functions.invoke('braip-consultar-vendas', {
        body: {
          mes,
      ...
      toast({
        ...
        description: `${(resp as Resultado).quantidade_vendida} venda(s) aprovadas no período.`,
      });
    } catch (e: any) {
      console.error(e);
      toast({ title: 'Erro na consulta Braip', description: e?.message || String(e), variant: 'destructive' });
    } finally {
      finalizarProgresso();
      setLoading(false);
    }
  ...
  const salvar = async () => {
    ...
    try {
      ...
      const { error } = await supabase.from('braip_consultas_salvas' as any).insert(payload);
      if (error) throw error;
  ...
  const exportarPdf = (origem?: any) => {
    ...
    let y = 18;
    doc.setFontSize(16);
    doc.text('Relatório de Comissão Braip', margemX, y);
    y += 8;
    doc.setFontSize(10);
    ...
    const linhas: [string, string][] = [
      ...
      ['Comissão real Braip', fmtBRL(Number(d.comissao_total))],
    ...
    if (porProduto.length) {
      autoTable(doc, {
        ...
        body: porProduto.map((p: any) => [
          p.nome, String(p.quantidade),
          fmtBRL(Number(p.faturamento)), fmtBRL(Number(p.comissao)),
        ]),
        theme: 'grid',
    ...
    doc.save(`comissao_braip_${nome}_${d.mes}.pdf`);
  };
    ...
    <Card>
      <CardHeader>
        <CardTitle className="text-base flex items-center gap-2">
          <TrendingUp className="h-4 w-4" /> Faturamento real Braip
        </CardTitle>
        <p className="text-xs text-muted-foreground">
          Consulta a API da Braip para apurar vendas aprovadas, quantidade e comissão real do mês selecionado. Filtre por nome de produto (parcial) ou pelo código exato.
        </p>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
          ...
          <div className="space-y-1 sm:col-span-2">
            ...
            <Input placeholder="ex.: Whey, Creatina..." value={produtoNome} onChange={(e) => setProdutoNome(e.target.value)} />
          </div>
          <div className="space-y-1">
            <Label>Código do produto (opcional)</Label>
            <Input placeholder="ex.: prod_XXXX" value={produtoCodigo} onChange={(e) => setProdutoCodigo(e.target.value)} />
          </div>
        </div>
        <div className="flex justify-end">
          <Button onClick={consultar} disabled={loading}>
            {loading
              ...
              : <><Search className="h-4 w-4 mr-2" />Consultar Braip</>}
          </Button>
        </div>
            ...
            <CardContent className="pt-5 space-y-4">
              <div className="flex items-center justify-between gap-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-semibold">Consultando Braip…</span>
                </div>
              ...
              <ul className="space-y-2">
                {etapas.map((e, i) => {
                  ...
                  return (
                    <li key={e.label} className={`flex items-center gap-2 text-xs transition-colors ${concluido ? 'text-emerald-600' : ativo ? 'text-primary font-medium' : 'text-muted-foreground'}`}>
              ...
              <Card>
                ...
                <CardContent>
                  ...
                  <div className="text-xs text-muted-foreground">
                    {data.total_retornado_api} retornadas · {data.total_aprovadas ?? data.quantidade_vendida} aprovadas
                  </div>
                </CardContent>
              </Card>
              <Card>
...
export default BraipConsultaCard;