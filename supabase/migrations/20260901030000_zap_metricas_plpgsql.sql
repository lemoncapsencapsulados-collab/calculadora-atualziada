-- Impede o inlining de `zap_metricas_consultor` convertendo-a para plpgsql.
--
-- Medido: 287 ms chamando de dentro do banco, contra 8,3 s pela API REST — 30x
-- mais lenta para o MESMO trabalho, com todas as etapas de acesso a dados
-- somando apenas 112 ms. O gargalo nunca esteve na consulta.
--
-- A causa é o inlining: o planejador expande funções `language sql` dentro da
-- consulta que as chama. Ao mesclar esta função com o invólucro que o PostgREST
-- monta, as CTEs deixam de funcionar como barreira de otimização e o plano
-- degenera. Funções plpgsql nunca são inlined.
--
-- Registro honesto: as duas otimizações anteriores (CTE única e turnos
-- materializados) atacaram o alvo errado. Continuam valendo por si — a tabela
-- de turnos evita recalcular window functions a cada leitura — mas não era ali
-- que estavam os 8 segundos.
