-- Agregador determinístico: UM objeto com todas as métricas do relatório.
--
-- Resolve a §2.1 da especificação. Hoje o parecer cita números que não existem
-- em lugar nenhum — não porque o modelo inventou, mas porque eu calculava
-- valores derivados dentro do prompt e não os persistia. Sem objeto único, não
-- há como validar texto contra dado.
--
-- A partir daqui: a tela, o PDF, o PPTX e o prompt de síntese consomem TODOS
-- este mesmo JSON. Se um número aparece no texto e não está aqui, é bug.
--
-- Tudo é derivado do que já está gravado. Nenhuma chamada de IA, nenhum custo.
-- Sinais que a especificação assumia precisar de extração nova — preço antes de
-- qualificar, link sem contexto, quem abandonou, respostas repetidas — saem de
-- regex sobre as mensagens cruas.
...
begin
  ...
  comportamento as materialized (
    select
      ...
      count(distinct (instance_name, remote_jid)) filter (
        where from_me and ordem_do_lado <= 3
          and texto ~* '(r\$ *[0-9]|[0-9]+ *reais|valor unit|pre[çc]o|tabela de pre)'
      ) as com_preco_cedo,
  ...
  proximo_passo as materialized (
    select count(*) as n from (
      select distinct on (instance_name, remote_jid) instance_name, remote_jid, texto, from_me
      from msgs order by instance_name, remote_jid, momento desc
    ) u
    where u.from_me and u.texto ~* '(amanh[ãa]|segunda|ter[çc]a|quarta|quinta|sexta|s[áa]bado|[0-9]{1,2}/[0-9]{1,2}|[àa]s [0-9]{1,2}|retorno|te (chamo|aviso)|combinado para)'
  ),
  ...
  repetidas as materialized (
    select lower(regexp_replace(btrim(texto), '[^a-zà-ú0-9 ]', '', 'g')) as trecho,
