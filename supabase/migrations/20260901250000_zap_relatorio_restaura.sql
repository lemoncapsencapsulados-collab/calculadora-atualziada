-- Restaura `zap_relatorio` à versão íntegra.
--
-- A migration 20260901210000 tentou enxertar o bloco de score dentro desta
-- função por substituição de texto. Uma das duas âncoras não existia mais (fora
-- reescrita na 200000), então só metade entrou: o JSON do score foi inserido
-- referenciando `velocidade.sla_15min_pct`, coluna que só existe no jsonb final
-- daquela CTE, não nela. Resultado: a função inteira passou a falhar com
-- "column sla_15min_pct does not exist".
--
-- Lição registrada: editar função de 260 linhas por `replace` de string é
-- frágil — quando a âncora some, o erro não aparece na hora da edição, aparece
-- em produção. O score virou função própria (zap_score) justamente por isso.
