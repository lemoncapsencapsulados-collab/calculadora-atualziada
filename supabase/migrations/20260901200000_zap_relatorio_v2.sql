-- Corrige três heurísticas do agregador, todas flagradas por resultado implausível.
--
-- 1. "Próximo passo definido" achou 5 casos em 357 conversas. A regra olhava a
--    ÚLTIMA mensagem da conversa; em 68 delas quem fala por último é o cliente,
--    então o combinado feito antes ficava invisível. Passa a olhar a última fala
--    DO CONSULTOR.
--
-- 2. "Respostas repetidas" devolvia URLs com pontuação removida
--    ("httpscatalogolemoncapscombr") — ilegíveis, e link repetido é distribuição
--    de catálogo, não roteiro de atendimento. Mensagens que são essencialmente
--    um link saem da conta.
--
-- 3. "Preço antes de qualificar" olhava só as 3 primeiras falas; passa a 5.
--    Verificado que o zero anterior era REAL, não falha da regex: o consultor
--    manda tabela de preço em texto (3% das mensagens dele), mas nunca na
--    abertura. Continua cego a preço falado em áudio.
